...
def get_overlaps(bcwindowsx, bcwindowsy, nbcs, logger):
    ...
    for i in range(0, height, 4):
        ...
        hist.append([curcounts, len(bcwindowsy[i]), len(bcwindowsx[j]), list(bcwindowsy[i+1])[0], list(bcwindowsy[i+1])[1], list(bcwindowsx[i+3])[0], list(bcwindowsx[i+3])[1]])
